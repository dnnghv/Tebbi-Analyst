...
def get_odoo_leads(date_from=None, date_to=None, state=None, tags=None, limit=1000):
    ...
    payload = {
        ...
        "params": {
            ...
            "kwargs": {
                "domain": domain,
                "fields": ["id", "create_date", "stage_id", "tag_ids", "name"],
                "limit": limit
            }
        },
        "id": 2
...
def odoo_lead_page():
    ...
    st.subheader("Thống kê số lượng Lead trên Odoo")
    ...
    with col3:
        odoo_state = st.text_input("Trạng thái (stage_id)", value="", key="odoo_state")
    odoo_tags = st.text_input("Tags (cách nhau bởi dấu phẩy)", value="", key="odoo_tags")
    tags_list = [int(tag.strip()) for tag in odoo_tags.split(',') if tag.strip().isdigit()]
    if st.button("🔍 Thống kê Lead Odoo"):
        with st.spinner("Đang lấy dữ liệu lead từ Odoo..."):
            df, err = get_odoo_leads(
                date_from=odoo_date_from,
                date_to=odoo_date_to,
                state=int(odoo_state) if odoo_state.isdigit() else None,
            ...
            if err:
                ...
                # By tag
                tag_list = df['tag_ids'].explode()
                tag_counts = tag_list.value_counts().reset_index()
                tag_counts.columns = ['Tag ID', 'Leads']
                fig3 = px.bar(tag_counts, x='Tag ID', y='Leads', title='Số lượng lead theo tag')
                st.plotly_chart(fig3, use_container_width=True)
                # Hiển thị bảng dữ liệu lead trực tiếp
                df['tag_ids'] = df['tag_ids'].apply(lambda x: ', '.join(map(str, x)) if isinstance(x, list) else str(x))
                st.markdown('### 📋 Bảng dữ liệu Lead')
                st.dataframe(df[['id', 'name', 'create_date', 'tag_ids']], use_container_width=True)
            else:
